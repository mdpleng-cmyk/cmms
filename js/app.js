import { sb, isRecoveryLink } from './store.js';
...
window.addChecklistItemToClass = addChecklistItemToClass;
...
window.saveTextValue = saveTextValue;
...
window.viewCreatedWorkOrder = viewCreatedWorkOrder;
...
window.cancelEditVisit = cancelEditVisit;
window.saveVisitEdit = saveVisitEdit;
...
window.saveRunnerText = saveRunnerText;
...
window.backToTypesList = backToTypesList;
window.saveTypeTemplateMeta = saveTypeTemplateMeta;
...
window.startEditTypeTemplateItem = startEditTypeTemplateItem;
...
window.startChecklistDrag = startChecklistDrag;
window.allowChecklistDrop = allowChecklistDrop;
window.dropChecklistItem = dropChecklistItem;
...
if (!isRecoveryLink && !isInPasswordRecovery()) {
  sb.auth.getSession().then(({ data }) => {
    if (data.session && !isInPasswordRecovery()) onSignedIn(data.session.user);
  });
}
