...
**`work_orders.planned_date`**: settable in both the New WO form and the Update modal, behind a "+ Set a planned date" toggle (auto-expanded if already set). A WO with a *future* planned_date remains visible in the Work Orders page but is omitted from Overview's Open Work Orders section until that date. It is also exempt from staleness tracking; once the date passes, staleness starts counting fresh **from the planned date**, not the original creation time.
...
**Equipment classes ("is there more than one of this?") — LIVE-LINKED as of `20260914090000_live_link_pm_templates.sql`**: `equipment_types` holds a class like "AC" or "FFS". `equipment_type_pm_templates`/`_items` define that class's PM checklist. New assets are still stamped at creation by `stamp_pm_template_on_asset` (`AFTER INSERT ON assets`), which now also sets `recurring_schedules.pm_template_id` and `checklist_items.template_item_id` so the new rows stay linked. **Editing the template after creation now propagates live** to every already-linked asset, via triggers on `equipment_type_pm_templates` / `equipment_type_pm_template_items` (`propagate_pm_template_change`, `propagate_pm_template_item_change`): add/edit/delete a template item in Manage → Equipment Types and it adds/edits/(soft-)deletes on every linked machine's checklist immediately; editing the template's title/interval/reminder does the same to every linked schedule (deliberately excluding `next_due_at`, so due dates never jump as a side effect of a template edit).
Reversed from the original "stamp, not link" decision after clarifying the old GAS Template+Asset Matrix problem was GAS being awkward to build sync logic in, not a flaw in live-linking itself. Two safeguards carried over from that earlier design conversation: (1) a `checklist_items` row with `template_item_id IS NULL` is asset-specific and is never touched by propagation — this is how a machine keeps a one-off task; (2) template item deletion soft-deletes (`active=false`) linked `checklist_items` rather than hard-deleting, so `wo_checklist_results` history is never orphaned. Pre-migration rows were backfilled/linked by matching (equipment class, title, interval_days) for schedules and (description, item_type, unit) for items — anything ambiguous was left unlinked rather than guessed. **Initial `next_due_at`** is still set to `NEW.created_at::date + interval_days` (the asset's own creation date). Subsequent recurrence after a PM completion still uses `completed_at::date + interval_days`.

**New Asset form flow** (user-designed, don't simplify away): a checkbox ("Do you have multiple similar items?") reveals a class dropdown with inline "+ Create new class..." — no detour to Manage required. Picking/creating a class auto-suggests a name (`"<Class> <N+1>"`) if the name field is still empty. Defining a class's PM checklist still happens only in Manage → Equipment Types, and edits there are now live-linked (see above).

**Explicitly NOT built** (proposed, pushed back on as over-complex, deferred): per-asset "reassign class" dropdown in Manage, bulk multi-select class assignment. If wanted later, treat as a fresh, smaller ask.

**Class-wide "Add to all units" — one-off, NOT template-linked (built)**: each multi-unit class group in the PMs tab (`js/schedules.js`, `addChecklistItemToClass()`) has an inline form that inserts one new `checklist_items` row (with `template_item_id` left NULL) into every schedule in that group at once. Use this for a task specific to the current batch of machines that should NOT become part of the class template or apply to future new assets — for anything that should be permanent for the class, add it to the template in Manage → Equipment Types instead, where it's live-linked. Skips schedules that already have an active item with the same description (case-insensitive), so re-clicking after a partial failure is safe.
